 ...
 *
 * Deliberately NOT persisted: a `File` can't be serialized to
 * `browser.storage.local`, and re-picking it after a reload/restart is the
 * only option anyway. Requests only ever reference a file by name in
 * storage; the bytes exist purely for the current session.
 */
interface FileState {
