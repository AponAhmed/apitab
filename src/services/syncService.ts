 ...
 * optimistically pre-setting `pushedVariableVersions` used to: if a create
 * failed (offline, timeout, 5xx — anything but a real conflict),
 * `pushedVariableVersions` still had no entry for it, so the *next* edit
 * saw `known` present some other way... actually the bug was the reverse:
 * it WAS set before the request even started, so a failed create looked
 * identical to a successful one afterward, and every future edit retried
 * it as an *update* against a row that was never created — a 404 that
 * never surfaces and never recovers. `pushedVariableVersions` now only
 * ever changes on a confirmed server response (success or conflict).
 */
...
export function initSyncService(): void {
  ...
  browser.storage.onChanged.addListener((changes, area) => {
    ...
    if ('apitab:collections' in changes) {
      applyingRemote = true;
    ...
    if ('apitab:teamVariables' in changes) {
      void useTeamVariablesStore.persist.rehydrate()?.finally(() => {
        const teamId = useTeamStore.getState().activeTeamId;
        if (!teamId) return;
        for (const v of useTeamVariablesStore.getState().variablesByTeam[teamId] ?? []) {
          if (!pushedVariableVersions.has(v.id)) pushedVariableVersions.set(v.id, { teamId, updatedAt: v.updatedAt });
        }
      });
    }
  });
...
// per team per cold start (fresh app launch / service worker respawn), not
// on every periodic poll tick.
